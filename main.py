...
response = requests.get("https://data.medicare.gov/resource/rbry-mqwu.json")
...
def detail():
    ...
    response = requests.get("https://maps.googleapis.com/maps/api/place/findplacefromtext/json?key=XXXXX&inputtype=textquery&fields=place_id&input=" + hospitalDB[int(firmid)]["name"])
    try:
        placeid = json.loads(response.text)["candidates"][0]["place_id"]
    except:
        return redirect("/")
    finalresponse = json.loads(requests.get(
        "https://maps.googleapis.com/maps/api/place/details/json?key=XXXXX&placeid=" + placeid).text)[
        "result"]
